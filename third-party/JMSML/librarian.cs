...
namespace JointMilitarySymbologyLibrary
{
    ...
    public class Librarian
    {
        ...
        // It sets up a _logger (NLog) for use throughout the code base.
        ...
        private SortedDictionary<ushort, SymbolSet> _sortedSymbolSets = new();
        private List<SymbolSet> _symbolSets = new();
        ...
        public Librarian(ILogger logger, string configPath = "")
        {
            ...
            XmlSerializer serializer = new(typeof(JMSMLConfig));
            ...
            else
            {
                ...
                // Use the file name from the parameter, if any
                if (string.IsNullOrWhiteSpace(configPath))
                {
                    // Fall back to default config file name
                    _configPath = Path.Combine(s, "jmsml.config");
                }
                else
                {
                    // Assume it is a relative path
                    _configPath = Path.Combine(s, configPath);
                }
            }

            if (File.Exists(_configPath))
            {
                using(FileStream fs = new(_configPath, FileMode.Open, FileAccess.Read))
                {
                    if (fs.CanRead)
                    {
            ...
            if(_configData is not null)
            {
                ...
                // Normalize backslashes from config to platform-specific separator (Linux compatibility)
                if (_configData.LibraryPath != null)
                    _configData.LibraryPath = _configData.LibraryPath.Replace('\\', Path.DirectorySeparatorChar);
                // Look for library under the specified path
                string path = Path.Combine(_configData.LibraryPath, _configData.LibraryName);
                if (!File.Exists(path))
                {
                    // Assume it is relative, and try to see if it under the assembly location
                    string root = Path.Combine(
                        Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location),
                        _configData.LibraryPath);
                    path = Path.Combine(root, _configData.LibraryName);
                    if (File.Exists(path))
                    {
                        // Assume that this is the location of the remaining JMSML artifacts
                        _configData.LibraryPath = root;
                    }
                    else
                    {
                ...
                using (FileStream fsLibrary = new(path, FileMode.Open, FileAccess.Read))
                {
                    ...
                    foreach (LibraryDimension dimension in this._library.Dimensions)
                    {
                        foreach (LibraryDimensionSymbolSetRef ssRef in dimension.SymbolSets)
                        {
                            ...
                            {
                                path = Path.Combine(_configData.LibraryPath, ssRef.Instance);
                                if (File.Exists(path))
                                {
                                    using (FileStream fsSymbolSet = new(path, FileMode.Open, FileAccess.Read))
                                    {
        ...
        internal void LogConversionResult(string converting)
        {
            if (_logConversion)
            {
                int unresolved = 0 - _statusFlag;

                // Bits 12-16 (Entity Type / Entity SubType / Modifier One / Modifier Two /
                // Legacy Symbol) are non-fatal: Symbol._ValidateStatus does not treat them
                // as invalidating, and they are reported routinely for tactical graphics and
                // for units that legitimately carry no subtype or modifier (the "----" code
                // positions). Only emit the conversion report when a *fatal* field could not
                // be resolved (bits 0-11, the same set _ValidateStatus uses to mark a symbol
                // invalid), so the log isn't flooded with benign "Not Found" diagnostics.
                const int fatalMask = 0xFFF; // bits 0..11
                if ((unresolved & fatalMask) == 0)
                {
                    return;
                }

                string s = Convert.ToString(unresolved, 2);
                char[] bits = s.PadLeft(17, '0').ToCharArray();
                Array.Reverse(bits);

                _logger.Info("----------------------------");
                _logger.Info("Converting: " + converting +  " (" + _statusFlag + ")");

                for (int i = 0; i < 17; i++)
                {
                    if (bits[i] == '1')
                    {
                        _logger.Info(_statusMessages[i]);
                    }
                }
        ...
        internal LibraryAmplifier SymbolAmplifier(string id)
        {
            ...
            _logger.Warn("Label " + id + " was not found in the base library file.");
        ...
        internal LibraryDrawRule DrawRule(string id)
        {
            ...
            _logger.Warn("DrawRule " + id + " was not found in the base library file.");
        ...
        internal LibraryDimension DimensionByLegacyCode(string code, string firstLetterInFunction = "", string codingSchemeLetter = "")
        {
            ...
            foreach (LibraryDimension lObj in this._library.Dimensions)
            {
                if (lObj.LegacyDimensionCode is not null)
                {
        ...
        internal SymbolSet SymbolSet(string dimensionID, string legacySIDC)
        {
            ...
            foreach (SymbolSet lObj in this._symbolSets)
            {
                ...
                {
                    if (lObj.LegacySymbols is not null)
                    {
        ...
        internal LibraryStatus Status(string code, string codingScheme)
        {
            ...
            foreach (LibraryStatus lObj in this._library.Statuses)
            {
                if (lObj.LegacyStatusCode is not null)
                {
        ...
        internal LibraryHQTFDummy HQTFDummy(string code)
        {
            ...
            foreach (LibraryHQTFDummy lObj in this._library.HQTFDummies)
            {
                if (lObj.LegacyHQTFDummyCode is not null)
                {
        ...
        internal LibraryAmplifierGroupAmplifier Amplifier(string code, string codingScheme)
        {
            ...
            foreach (LibraryAmplifierGroup lObj in this._library.AmplifierGroups)
            {
                if (lObj.Amplifiers is not null)
                {
                    foreach (LibraryAmplifierGroupAmplifier lObj2 in lObj.Amplifiers)
                    {
                        if (lObj2.LegacyModifierCode is not null)
                        {
        ...
        internal LibraryAffiliation AffiliationByLegacyCode(string legacyStandardIdentityCode, string dimensionID)
        {
            ...
            foreach (LibraryAffiliation lObj in this._library.Affiliations)
            {
                if (lObj.DimensionID == dimensionID && lObj.LegacyStandardIdentityCode is not null)
                {
        ...
        //    foreach(LibraryAffiliation lObj in this._library.Affiliations)
        //    {
        //        if (lObj.LegacyStandardIdentityCode is not null)
        //        {
        //            // For each standard identity
        ...
        internal SymbolSetEntity Entity(SymbolSet symbolSet, ushort entityCodeOne, ushort entityCodeTwo)
        {
            ...
            if (symbolSet is not null)
            {
                if (symbolSet.Entities is not null)
                {
        ...
        internal SymbolSetEntityEntityType EntityType(SymbolSetEntity entity, ushort entityTypeCodeOne, ushort entityTypeCodeTwo)
        {
            ...
            {
                if (entity.EntityTypes is not null)
                {
        ...
        internal EntitySubTypeType EntitySubType(SymbolSetEntityEntityType entityType, ushort entitySubTypeCodeOne, ushort entitySubTypeCodeTwo)
        {
            ...
            {
                if (entityType.EntitySubTypes is not null)
                {
        ...
        internal EntitySubTypeType EntitySubType(SymbolSet ss, ushort entitySubTypeCodeOne, ushort entitySubTypeCodeTwo)
        {
            ...
            if (ss is not null)
            {
                if (ss.SpecialEntitySubTypes is not null)
                {
        ...
        internal ModifiersTypeModifier ModifierOne(SymbolSet symbolSet, ushort modifierCodeOne, ushort modifierCodeTwo)
        {
            ...
            if (symbolSet is not null)
                if (symbolSet.SectorOneModifiers is not null)
                {
        ...
        internal ModifiersTypeModifier ModifierTwo(SymbolSet symbolSet, ushort modifierCodeOne, ushort modifierCodeTwo)
        {
            ...
            if (symbolSet is not null)
                if(symbolSet.SectorTwoModifiers is not null)
                {
                ...
                if (lObj.LegacyDimensionCode is not null)
                {
                    foreach (LegacyLetterCodeType lObj2 in lObj.LegacyDimensionCode)
                    {
                        if ((lObj2.Value == code && lObj2.FirstFunctionLetter == "" && lObj2.CodingSchemeLetter == "") ||
                            ...
                            (lObj2.Value == code && lObj2.FirstFunctionLetter == firstLetterInFunction && lObj2.CodingSchemeLetter == codingSchemeLetter))
                        {
                            if (lObj.SymbolSets is not null)
                            {
                                foreach (LibraryDimensionSymbolSetRef ssRef in lObj.SymbolSets)
                                {
        ...
        internal SymbolSetLegacySymbol LegacySymbol(SymbolSet symbolSet, string legacySIDC)
        {
            ...
            string tailX = legacySIDC.Substring(12, 2) + "X"; // Legacy TGs may have an "X" order of battle when STP's doesn't

            if (symbolSet is not null)
            {
                if (symbolSet.LegacySymbols is not null)
                {
                        ...
                        foreach (LegacyFunctionCodeType lObj2 in lObj.LegacyFunctionCode)
                        {
                            if (lObj2.Value == functionCode)
                            {
                                ...
                                if ((lObj2.SchemaOverride == "" || (lObj2.SchemaOverride != "" && lObj2.SchemaOverride == schema)) &&
                                    ...
                                    (lObj2.TailOverride == "" || (lObj2.TailOverride != "" && (lObj2.TailOverride == tail || lObj2.TailOverride == tailX))))
                                {
                                    return lObj;
                                }
                            }
        ...
        public Symbol MakeSymbol(string legacyStandard, string legacySIDC)
        {
            ...
            if (legacySIDC.Length == 15)
            {
                ...
                {
                    _logger.Warn("SIDC " + legacySIDC + " is an invalid symbol.");
                    s = null;
                }
            }
            else
            {
                _logger.Error("SIDC " + legacySIDC + " is not 15 characters in length.");
            }
        ...
        {
            SIDC sid = new(partA, partB);
            Symbol s = new(this, sid, _drawColoredOCABars, _drawCivilianFrames);
        ...
        {
            Symbol s = new(this, sidc, _drawColoredOCABars, _drawCivilianFrames);
