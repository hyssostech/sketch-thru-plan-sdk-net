...
namespace JointMilitarySymbologyLibrary
{
    ...
    public class ConfigHelper
    {
        ...
        private Dictionary<string, string> _framePaths = new();
        private Dictionary<string, string> _entityPaths = new();
        private Dictionary<string, string> _modifierOnePaths = new();
        private Dictionary<string, string> _modifierTwoPaths = new();
        private Dictionary<string, string> _basePaths = new();
            ...
            if (inThis is not null)
            {
                ...
                switch (findThis)
                {
                    ...
                    case FindEnum.FindEntities:
                        if (inThis.Entities is not null)
                        {
                    ...
                    case FindEnum.FindModifierOnes:
                        if (inThis.ModifierOnes is not null)
                        {
                    ...
                    case FindEnum.FindModifierTwos:
                        if (inThis.ModifierTwos is not null)
                        {
                ...
                else
                {
                    ...
                    if (inThis.GraphicFolder is not null)
                    {
        ...
        private void _findPath(FindEnum findThis, string forThis, Dictionary<string, string> recordItHere)
        {
            ...
            //if(!foundIt)
            //    _logger.Warn("Can't find the " + findThis + " config data for " + forThis + " graphics.");
        }
        ...
        public string BuildOriginalPath(string path, string graphic)
        {
            ...
            string result = this.GraphicHome + path + "\\" + graphic;
            // Add library root if file not found
            if (!File.Exists(result))
                result = Path.Combine(_configData.LibraryPath, result); 
        ...
        public LegacyLetterCodeType[] LegacyLetters(LegacyLetterCodeType[] letterArray, string standard)
        {
            ...
            List<LegacyLetterCodeType> letterCodes = new();
        ...
        public LegacyFunctionCodeType[] LegacyFunctions(LegacyFunctionCodeType[] functionArray, string standard)
        {
            ...
            List<LegacyFunctionCodeType> functionCodes = new();
        ...
        public bool IsSpecialEntitySubtype(SymbolSet ss, EntitySubTypeType entitySubType)
        {
            ...
            if (ss is not null && entitySubType is not null)
            {
        ...
        public string MapKey(string standard, string key)
        {
            ...
            if (_etlConfig.KeyMaps is not null)
            {
